...
def movies_with_large_cast (matches: List[str]) -> List[str]:
    min_actors = int(matches[0])
    results =[] 
    for movie in movie_db:
...
pa_list: List[Tuple[List[str], Callable[[List[str]], List[Any]]]] = [
    ...
    (str.split("what movies have more than % cast members?"), movies_with_large_cast),
    (["bye"], bye_action),
]
...
def query_loop() -> None: #dont do this
...
if __name__ == "__main__":
    ...
    assert sorted(title_before_year(["1950"])) == sorted(["casablanca", "citizen kane", "gone with the wind", "metropolis"]
    ), "failed title_before_year test"
    assert isinstance(title_after_year(["1990"]), list), "title_after_year not returning a list"
    assert sorted(title_after_year(["1990"])) == sorted(["boyz n the hood", "dead again", "the crying game", "flirting", "malcolm x"]
    ...
    assert isinstance(movies_with_large_cast(["3"]), list), "movies_with_large_cast not returning a list"
    assert sorted(movies_with_large_cast(["5"])) == sorted(["jaws"]) 
